...
class appqoe_system(object):
    ...
    def test_Scale_scenario3(self,device='pm9008'):
        flag = 0
        #***********CleanUp*********
        cleanup     =   self.deactivate_and_delete_localizedPolicy()
        if cleanup[0]:
    ...
    def test_Scale_scenario4(self,device='pm9008'):
        flag = 0
        #***********CleanUp*********
        cleanup     =   self.deactivate_and_delete_localizedPolicy()
        if cleanup[0]:
    ...
    def deactivate_and_delete_localizedPolicy(self):
        #***Deactivate*****
    ...
    def fnf_drop(self,device='pm9009'):
        cmd="show sdwan app-fwd cflowd flows format table"
        dest_ip = self.topology.mgmt_ipaddr(device)
        no_timeout = self.confd_client.sendline(dest_ip, "terminal length 0",timeout=800)
        output = self.confd_client.sendline(dest_ip, cmd)
        output = output['message']
        if len(output) == 0:
    ...
    #@run.test(['test_appqoe_libuinet_stats'])
    ...
    def verify_sdwan_appqoe_flows(self,device, vrfvalue, server_port='80'):
